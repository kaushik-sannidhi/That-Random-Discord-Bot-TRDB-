...
client = commands.Bot(command_prefix="trdb ", intents=intent)
...
async def help(ctx):
  if (selfBot):
    ...
    em = discord.Embed(title="Help Menu",
                       description="Use: trdb help to get the help menu")
...
async def mag8ball(ctx):
  if (selfBot):
    ...
    em = discord.Embed(title="Mag8ball",
...
async def bucketlist(ctx):
  if (selfBot):
    ...
    em = discord.Embed(
        title="Bucketlist",
        description="Displays an item you should add to your bucketlist",
        inline=False)
...
async def chucknorris(ctx):
  if (selfBot):
    ...
    em = discord.Embed(title="Chucknorris",
                       description="Displays a chuck norris joke",
                       inline=False)
...
async def hobbies(ctx):
  if (selfBot):
    ...
    em = discord.Embed(title="Hobbies",
                       description="Displays a hobby",
                       inline=False)
...
async def trivia(ctx):
  if (selfBot):
    ...
    em = discord.Embed(title="Trivia",
                       description="Displays a trivia",
                       inline=False)
...
async def riddle(ctx):
  ...
  response = requests.get(
      api_url,
      headers={'X-Api-Key': 'yrrwohytFsUz2YCFNqL2Hw==MxUiXJFgFM8j35Np'})
...
async def on_message_edit(message_before, message_after):
  ...
  if (author != 728263722598137989):
    await channel.send(
        f"""Pre-edit: `{message_before.content}`\nPost-edit: `{message_after.content}`\nAuthor: <@!{message_before.author.id}>"""
    )
